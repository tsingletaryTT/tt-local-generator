...
class MochiPipeline(DiffusionPipeline):
    ...
    def create_pipeline(
        ...
        if ttnn.device.is_blackhole():
            assert tuple(mesh_device.shape) in [(2, 2), (2, 4), (4, 8)], "Mochi on BH supports (2,2), (2,4), or (4,8) mesh shapes"  # patched: (2,2) enables P300X2
            default_config = {
                (2, 2): {
                    "sp_axis": 0,
                    "tp_axis": 1,
                    "num_links": 2,
                    "vae_mesh_shape": (1, 4),  # patched: reshape to (1,4) so time_parallel.factor=4 (in core_grid_y_map); (2,2) gives factor=2 which is missing
                    "vae_sp_axis": 0,
                    "vae_tp_axis": 1,
